...
bool compararPesos(tuple<int,int,int,int>& x, tuple<int,int,int,int>& y){
    float c_1 = static_cast<float>(get<0>(x)) - C*static_cast<float>(get<1>(x));
    float c_2 = static_cast<float>(get<0>(y)) - C*static_cast<float>(get<1>(y));
    return c_1>c_2;
}
...
float kruskal(vector<tuple<int,int ,int,int>>& E, int n){ //código de la cátedra modificado
    ...
    for(auto arista : E){
        ...
        float c_i = static_cast<float>(d_i) - C*static_cast<float>(r_i);
        int u = get<2>(arista);
        int v = get<3>(arista);
    ...
    if(aristas == n-1) return res; // res = sum_{e \in T}(d_e - C*r_e)
}
...
pair<int,int> solOptima(vector<tuple<int,int,int,int>>& E, int n){ //código de la cátedra modificado
    ...
    if(aristas == n-1) return make_pair(sum_D,sum_R);
}
int main(){
    int tests;
    cin>>tests;
